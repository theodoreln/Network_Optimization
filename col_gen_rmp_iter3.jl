...
#LP solution
@variable(model, 0 <= x1[1:3] <= 1)  # x1[1], x1[2], x1[3], possible routes for commodity k=1, added one by one
@variable(model, 0 <= x2[1:3] <= 1)  
@variable(model, 0 <= x3[1:3] <= 1)
...
@objective(model, Min, 14 * x1[1] + 30 * x2[1] + 90 * x3[1] + (90-35)*x3[2])
...
@constraint(model, con9, 5*x3[1] <= 6) # y_DB
...
@constraint(model, y3, x3[1]+x3[2] == 1)                               # Constraint (y_3)
...
for i in 1:3
    ...
    println("x2[$i] = ", value(x2[i]))
    println("x3[$i] = ", value(x3[i]))
end
